...
If tries to define a basic interoperability layer between object editors and object renderers in a live production workflow.
...
Immersive audio is gaining ground in different industries, from music streaming to gaming, live sound to broadcast.
[ADM](https://adm.ebu.io/) or _Audio Definition Model_, is becoming a popular standard metadata model in some industries, with serial ADM used in broadcast or ADM xml files used in the studio.
...
![_SPAT Revolution_ ADM OSC](https://flux-shared.s3.amazonaws.com/doc_images/SpatR/Education/ADMOSC.png )

_SPAT Revolution_ supports **ADM-OSC** in input as an alternate grammar, and on output as an option. The specification calls for normalized (linear) data value to provide interoperability and tend to align with the ADM protocol. Object-based mixing renderers will typically handle the scaling based on the system configuration.
...
![OSC I/O presets](https://flux-shared.s3.amazonaws.com/doc_images/SpatR/Preference/OSCConnectionsADM.png )
...
Enter your desired value to modify the incoming range (the automation zone). In this example, we are scaling to -3.00, 3.00.

![OSC I/O presets]( https://flux-shared.s3.amazonaws.com/doc_images/SpatR/Preference/OSCTransformPresetsADM.png)
...
Enter your desired value to modify the incoming range (the automation zone).
In this example, we send data from the -3.00, 3.00 zone.
All output options are already set with the preset.

![OSC I/O presets](https://flux-shared.s3.amazonaws.com/doc_images/SpatR/Preference/OSCTransformPresetADMOutput.png)


## Third party example


Recently, L-Acoustics released their new L-ISA controller that can now output ADM-OSC as an alternate method (hardware required). For example, it is functional to be received by _SPAT Revolution_. OSC messages can be sent using the ADM-OSC format and be interpreted identically by any ADM-OSC compatible device.

![L-ISA](https://flux-shared.s3.amazonaws.com/doc_images/SpatR/ThirdParty/LisaOSCConfiguration.png)

Furthermore, Nuendo V11 adds the support of external OSC renderers, by mapping bidirectionally the multi-panner.

![Nuendo](https://flux-shared.s3.amazonaws.com/doc_images/SpatR/ThirdParty/NuendoPanner.png)