...
Maybe Ableton Live is your main DAW and so the most obvious choice, this is the case for me and is also why I created this workflow in the first place, but I would also like to add, that Live is a great choice for anyone getting into creating, sound designing, etc… in an Immersive Audio Context. 

The capabilities of Ableton Live in terms ofaAutomations are monstrous and so make this DAW an incredible tool for making great Immersive Audio content that will include lots of spatial movements and effects.
...
⚠️ DISCLAIMER: This workflow has been specifically thought of for maximum compatibility between Ableton Live and SPAT Revolution under macOS. Some parts of this guide *(mainly the devices provided)* can still be applied to Windows or/and other DAWs.
...
The Devices are installed on the following directory : <code>/Library/Application Support/Flux/Ableton Live Devices/SPAT”</code>
...
COMMENT: please indicate why we are not using the LAP if you want to keep this sentances, otherwise it's not really interesting to keep it
...
You can make it even more simply using the provided *Shortcuts package* I made to do just that. They are located in the same folder as the other device so: <code>/Library/Application Support/Flux/Ableton Live Devices/SPAT/macOS Shortcuts”</code>
...
To add the Shortcuts I made, simply double click each of them and select the <code>Add Shortcut</code> button.
...
!> If you work with MIDI controller, I also highly recommend that you go into the Live Preferences, into the MIDI tab and change the Takeover Mode to Value Scaling so that parameters you play with don't get too jumpy.
...
Here I have a session which I am going to render for the Binaural *(3d on headphones)* Version and at the same time generate a Bed for an ATMOS render later.
...
Then I will use this device as Output in SPAT Revolution and Input in Ableton Live, set the out channels of the binaural output block to the two corresponding channels of my 2ch device and the rest to the Atmos output.
...
Of course don’t forget to check if the OSC is enable in the preferences or SPAT will simply not receive anything.
...
The Immersive Audio World is full of new possibilities and I hope that this guide helped you understand lots of what spatial audio has to offer and how to utilize it with Ableton Live!
...
A: Maybe if I get lots of requests about it.
...
I see... you got completely hooked to making Immersive Audio, while this guide will be updated we already have a lot more resources you can checkout right away!
